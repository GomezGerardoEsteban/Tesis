...
###################################################################################################################################################################
# Titulo: Relación entre oferta y demanda eléctrica al 2032
# Tema: Este gráfico muestra el comportamiento medio entre oferta y demanda de energia electrica, de llevarse a cabo la totalidad de los 
#       proyectos de generación desde FNCER al 2032.
# Author: Gerardo Esteban Gomez-Santiago                                                                                                        
# Fecha: 9 de Julio de 2024                                                                                                                     
# Base de datos: Para realizar este gráfico fue necesario calcular los factores de planta por hora y por tipo de fuente a partir de información 
#                oficial sobre generación y capacidad instalada entre 2019 y 2023.
#                Utilizando las estimaciones de demanda de la UPME y la proporción de la demanda por hora, se obtiene el comportamiento esperado
...
colores <- c("black", "mediumorchid4", "lightcyan4", "green4", "navy", "lightblue3", "gold3")

bases_ge_dia %>% 
  ggplot() +
  geom_col(mapping = aes(x = hora, y = oferta_kWh/1000000, fill = Combustible_),
           show.legend = F,
           alpha = 0.8,
           color = "grey") +
  geom_point(mapping = aes(x = hora, y = escenario_dia_2032/1000000), shape = 23, color = "black", fill = "coral",
             size = 3) +
  geom_point(mapping = aes(x = 4, y = 22), shape = 23, color = "black", fill = "coral",
             size = 3)+
  annotate(geom = "text",
           x = 3, y = 22, label = "Demanda\nEléctrica",
           color = "coral",
           size = 3.5) +
  # geom_point(mapping = aes(x = hora, y = escenario_dia_2037), shape = 23, color = "black", fill = "coral") +
  scale_fill_manual(values = colores) +
  labs(title = "Comparación entre oferta eléctrica (según proyectos aprobados) y demanda eléctrica al 2032",
       subtitle = "Capacidad media de generacion <span style='color:gold3;'>Solar</span>, <span style='color:lightblue3;'>Eólica</span>, <span style='color:navy;'>Hidroeléctrica</span>, <span style='color:lightcyan4;'>Gas</span> y <span style='color:mediumorchid4;'>Carbón</span>",
       x = "Hora",
...
ggsave(filename = "rmd/resultados/graficos/grafico5.34_oferta_demanda.png",
       dpi = 500,
       width = 9,
       height = 5)
