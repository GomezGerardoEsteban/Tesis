...
#################################################################################################################################################
# Titulo: Relación entre crecimiento económico e intensidad eléctrica por sectores en la economía colombiana
# Tema: Este gráfico muestra como entre 2005 y 2021 los sectores mas intensivos en electricidad no son lo que mas crecieron
#       en la economía colombiana. Para ello utilizo los coeficientes tecnicos de producción (matriz A derivada de la matriz I-P)
#       y el calculo de las tasas de crecimiento equivalente anual entre 2005 y 2021.
...
graph_Crecimiento_Elec <- bases_solo_sec %>% 
  ...
  annotate(geom = "text",
           ...
           label = c(str_c("Crecimiento PIB = ", round(bases_macro_sec$tasa_equiv_anual[bases_macro_sec$Nombre == "PIB"]*100, 2), " %", sep = ""),
                     str_c("Media ponderada de\nIntensidad eléctrica = ", round(weighted.mean(x = bases_solo_sec$int_elec[-c(59,34)], w = bases_solo_sec$prop[-c(59,34)]), 4)*100, sep = "")),
           color = c("darkblue", "darkred"),
           size = 3) +
  labs(title = "Relación entre crecimiento económico e intensidad eléctrica según la matriz de coeficientes técnicos de producción",
...
# graph_Crecimiento_Elec

ggsave(filename = "rmd/resultados/graficos/grafico1.11_crecimiento_intElect.png",
       plot = graph_Crecimiento_Elec,
       width = 10,
       height = 5.81,
       dpi = 500)
