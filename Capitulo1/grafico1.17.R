...
grafico_deptos_estrato_proporcion <- estratosDepto %>% 
  filter(Estratos == 1 & Dept != "88") %>%
  ggplot() +
  ...
  geom_rect(mapping = aes(xmin = 25.5, xmax = 29.5,
                          ymin = 0, ymax = 20),
            fill = "lightblue",
            alpha = 0.01) +
  geom_rect(mapping = aes(xmin = 30.5, xmax = 32.5,
                          ymin = 0, ymax = 20),
            fill = "lightblue",
            alpha = 0.01) +
  scale_y_continuous(n.breaks = 10,
                     limits = c(0, 21)) +
  labs(title = "Proporción del ingreso de los hogares Estrato 1 destinado al pago de la electricidad en departamentos de Colombia - 2023",
       # subtitle = "",
       y = "Porcentaje",
       x = NULL,
...
ggsave(filename = "rmd/resultados/graficos/grafico1.17_deptos_estrato_proporcion.png",
       plot = grafico_deptos_estrato_proporcion,
       width = 8.45,
       height = 2.91,
       dpi = 500)
